...
{
    public class CidadeModelController
    {
        public CidadeViewModel GetById(int id)
        {
            ...
            if (cidade != null)
            {
                return new CidadeViewModel()
                {
                    Id = cidade.Id,
                    Nome = cidade.Nome,
                    Estado = new EstadoModelController().GetById(cidade.Estado)
                };
            }
        ...
        public List<CidadeViewModel> Get()
        {
            ...
            if (cidades != null && cidades.Count > 0)
            {
                return cidades.Select(cidade => new CidadeViewModel() {Id = cidade.Id, Nome = cidade.Nome, Estado = new EstadoModelController().GetById(cidade.Estado)}).ToList();
            }
